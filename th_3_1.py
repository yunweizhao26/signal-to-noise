...
def load_data(args):
    ...
    # Load AnnData object
    adata = read_h5ad(args.input_file, backed='r' if args.low_memory else None)
...
def log1p_cpm(mat, inplace=False):
    """
    Memory-efficient normalize matrix to log1p of counts per million
    ...
    inplace : bool
        Whether to modify the matrix in-place (only works for dense matrices)
    ...
    if is_sparse:
        ...
        # Efficient row sum for sparse matrix
    ...
    if is_sparse:
        import scipy.sparse as sp
        # For CSR matrices, we can scale each row efficiently
        if isinstance(mat, sp.csr_matrix):
            ...
            # Convert to CSR first for efficiency
            mat_csr = mat.tocsr()
        ...
        if inplace:
            ...
            for i in range(mat.shape[0]):
                mat_normalized[i, :] = mat[i, :] * scale[i]
    ...
    if is_sparse:
        ...
        # For dense matrices
        if inplace:
            np.log1p(mat_normalized, out=mat_normalized)
...
def calculate_th_scores_with_marker_weights(adata, method_names, marker_genes, args):
    ...
    if "disease" in adata.obs.columns and "tissue" in adata.obs.columns:
        ...
        for idx, (disease, tissue) in enumerate(metadata_markers.values):
            ...
            del adata  # AnnData object can be very large
            gc.collect()
...
def calculate_evaluation_metrics(X_original, X_imputed, sample_size=1000):
    """
    Calculate basic evaluation metrics to assess imputation quality.
    Uses sampling to handle large matrices efficiently.
    
    Parameters:
    -----------
    X_original : array or sparse matrix
        Original gene expression matrix
    X_imputed : array or sparse matrix
        Imputed gene expression matrix
    sample_size : int
        Number of cells/genes to sample for metrics calculation
    ...
    orig_sparse = hasattr(X_original, 'toarray')
    imp_sparse = hasattr(X_imputed, 'toarray')
    
    # Sample cells and genes for large matrices
    n_cells = min(sample_size, X_original.shape[0])
    n_genes = min(sample_size, X_original.shape[1])
    
    cell_indices = np.random.choice(X_original.shape[0], n_cells, replace=False)
    gene_indices = np.random.choice(X_original.shape[1], n_genes, replace=False)
    
    # Extract submatrices
    if orig_sparse:
        X_orig_sub = X_original[cell_indices, :][:, gene_indices].toarray()
    else:
        X_orig_sub = X_original[cell_indices, :][:, gene_indices]
    
    if imp_sparse:
        X_imp_sub = X_imputed[cell_indices, :][:, gene_indices].toarray()
    else:
        X_imp_sub = X_imputed[cell_indices, :][:, gene_indices]
    
    # Calculate metrics
    
    # 1. Correlation between original and imputed values
    nonzero_mask = X_orig_sub > 0
    if np.sum(nonzero_mask) > 0:
        metrics['nonzero_correlation'] = np.corrcoef(X_orig_sub[nonzero_mask], X_imp_sub[nonzero_mask])[0, 1]
    else:
        metrics['nonzero_correlation'] = 0
    
    # 2. Preservation of zeros
    zero_mask = X_orig_sub == 0
    if np.sum(zero_mask) > 0:
        metrics['zero_preservation'] = np.mean(X_imp_sub[zero_mask] == 0)
    else:
        metrics['zero_preservation'] = 1.0
    
    # 3. Mean absolute error for nonzeros
    if np.sum(nonzero_mask) > 0:
        metrics['nonzero_mae'] = np.mean(np.abs(X_orig_sub[nonzero_mask] - X_imp_sub[nonzero_mask]))
    else:
        metrics['nonzero_mae'] = 0
    
    # 4. Global correlation
    metrics['global_correlation'] = np.corrcoef(X_orig_sub.flatten(), X_imp_sub.flatten())[0, 1]
...
def incremental_imputation(adata, marker_genes, th_results, args):
    """
    Memory-efficient incremental imputation algorithm.
    ...
    for sample_idx, sample_data in enumerate(th_results):
        # Log memory usage
        log_message(f"Memory usage before processing sample {sample_idx+1}: {get_memory_usage():.1f} MB")
        ...
        log_message(f"Processing sample {sample_idx+1}/{len(th_results)}: {tissue} ({disease})")
        
        # Skip if too few zeros
        if len(zero_indices[0]) < 10:
            log_message(f"  Skipping: too few zeros ({len(zero_indices[0])})")
            continue
        
        # Create test matrix (copy of original)
        is_sparse = hasattr(X_original, 'toarray')
        ...
        log_message(f"  Sorting {len(scores)} zeros by TH score")
        sorted_indices = np.argsort(-scores)
        
        # Determine step size based on matrix sparsity
        total_zeros = len(sorted_indices)
        if args.batch_percent > 0:
            step_percent = args.batch_percent / 100.0
            step_size = max(1, int(total_zeros * step_percent))
        else:
            # Adaptive step size
            if total_zeros > 1000000:
                step_size = max(1, total_zeros // 100)  # 1% steps for very large matrices
            elif total_zeros > 100000:
                step_size = max(1, total_zeros // 50)   # 2% steps for large matrices
            else:
                step_size = max(1, total_zeros // 20)   # 5% steps for smaller matrices
        
        step_percent = step_size / total_zeros
        log_message(f"  Using step size: {step_size} zeros per step ({step_percent:.2%} of zeros)")
        
        # Initialize tracking
        imputation_curve = []
        
        # Calculate initial metrics
        log_message(f"  Calculating initial metrics")
        initial_metrics = calculate_evaluation_metrics(X_original, X_test)
        
        imputation_curve.append({
            "iteration": 0,
            "imputed_entries": 0,
            "imputed_fraction": 0.0,
            "composite_score": initial_metrics.get('global_correlation', 0),
            "correlation": initial_metrics.get('global_correlation', 0),
            "zero_preservation": initial_metrics.get('zero_preservation', 1.0),
            "mae": initial_metrics.get('nonzero_mae', 0),
            "metrics": initial_metrics
        })
        
        # Track best score
        best_score = initial_metrics.get('global_correlation', 0)
        best_iteration = 0
        ...
        for i in range(0, total_zeros, step_size):
            ...
            if is_sparse:
                import scipy.sparse as sp
                # For sparse matrices, update efficiently
                if isinstance(X_test, sp.csr_matrix):
                    # Convert to COO for updating
                    X_test_coo = X_test.tocoo()
                    # Add new nonzero entries
                    ...
                    # For other sparse formats, update directly
            ...
            # Calculate metrics
            log_message(f"  Calculating metrics for step {(i // step_size) + 1}")
            current_metrics = calculate_evaluation_metrics(X_original, X_test)
            
            imputed_frac = (i + len(batch_indices)) / total_zeros
            current_score = current_metrics.get('global_correlation', 0)
            ...
            imputation_curve.append({
                ...
                "correlation": current_metrics.get('global_correlation', 0),
                "zero_preservation": current_metrics.get('zero_preservation', 1.0),
                "mae": current_metrics.get('nonzero_mae', 0),
                "metrics": current_metrics,
                "improvement": current_score - initial_metrics.get('global_correlation', 0)
            })
            
            log_message(f"  Step {(i // step_size) + 1}: Imputed {imputed_frac:.1%}, Score: {current_score:.4f}" + 
                       (f" (improved by {current_score - initial_metrics.get('global_correlation', 0):.4f})" 
                         if current_score > initial_metrics.get('global_correlation', 0) else ""))
            ...
            if args.early_stop > 0:
                if current_score > (best_score + improvement_threshold):
                    ...
                    best_iteration = len(imputation_curve) - 1
                else:
                    no_improvement_count += 1
                
                if no_improvement_count >= args.early_stop:
                    log_message(f"  Early stopping after {no_improvement_count} iterations without improvement")
                    break
        ...
        log_message(f"  Optimal imputation: {max_score_frac:.1%} of zeros, score: {max_score:.4f}")
        
        # Create optimal matrix
        if max_score_idx > 0:  # If improvement found
            ...
            if is_sparse:
                ...
                # Update matrix
                if isinstance(X_optimal, sp.csr_matrix):
                    # Convert to COO for updating
                    X_optimal_coo = X_optimal.tocoo()
                    X_optimal_coo.data = np.concatenate([X_optimal_coo.data, opt_values])
                    X_optimal_coo.row = np.concatenate([X_optimal_coo.row, opt_rows])
                    X_optimal_coo.col = np.concatenate([X_optimal_coo.col, opt_cols])
                    X_optimal = sp.csr_matrix((X_optimal_coo.data, (X_optimal_coo.row, X_optimal_coo.col)), 
                                            shape=X_optimal.shape)
                else:
                    # For other sparse formats
        ...
        # Save optimal matrix
        output_file = os.path.join(args.output_dir, f"optimal_{disease}_{tissue}")
        
        if is_sparse:
            # Save sparse matrix
            import scipy.sparse as sp
            sp.save_npz(output_file + '.npz', X_optimal)
            log_message(f"  Saved optimal sparse matrix to {output_file}.npz")
        else:
            # Save dense matrix
            np.save(output_file + '.npy', X_optimal)
            log_message(f"  Saved optimal matrix to {output_file}.npy")
        
        # Store results without matrices to save memory
        sample_result = {
            "disease": disease,
            "tissue": tissue,
        ...
        log_message(f"  Memory usage after processing: {get_memory_usage():.1f} MB")
...
def main():
    ...
    parser.add_argument("--low_memory", action="store_true", 
                    help="Enable low memory optimizations (memory mapping, etc.)")
    parser.add_argument("--max_zeros_to_process", type=int, default=1000000, 
                    help="Maximum number of zeros to process (sample if more)")
